...
if [ $# -ge 1 ] ; then
 ...
 if [ $USER == "ngu" ] ; then
  TOUTATISPATH=$HOME/Toutatis.app/Contents/MacOS
 fi
fi
...
vanefile="./toutatis.vane"

if [ -f "$vanefile" ]
then
echo theGeometryFileFlag 1 ./toutatis.vane >> myex.inp
fi

if [ -f ../gap.tmp ] ; then
 cat ../gap.tmp >> myex.inp
else
 echo end >> myex.inp
fi
...
if [ -f "$vanefile" ]
then
cp toutatis.vane ../toutatis_result
fi
cd ..
cd toutatis_result
